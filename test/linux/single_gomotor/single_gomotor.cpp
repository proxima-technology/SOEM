        ...
        if (ec_config_init(FALSE) > 0) {
            ...
            if (ec_slave[0].state == EC_STATE_OPERATIONAL) {
                ...
                do {
                    ...
                    ec_receive_processdata(EC_TIMEOUTRET);
                    hash_check_cnt++;
                } while (strncmp(vcheck, (char*)ec_slave[1].inputs, sizeof(vcheck) / sizeof(vcheck[0])) != 0 && hash_check_cnt < 1000);

                int verlen = ec_slave[1].inputs[sizeof(vcheck) / sizeof(vcheck[0])];
                uint8_t* slave_ver;
                slave_ver = (uint8_t*)malloc(verlen * sizeof(uint8_t));
                memcpy(slave_ver, (char*)ec_slave[1].inputs + sizeof(vcheck) / sizeof(vcheck[0]) + 1, verlen);
